...
from events.serializers import DebitorListSerializer
from events.utils import get_event_object_by_uuid, get_event_attendee_object_by_uuid
from orders.models import Order
from orders.utils import get_net_positive_balance_credit_from_latest_order, get_latest_order_of_main_user


class DebitorListView(ListAPIView, CustomPagination):
    serializer_class = DebitorListSerializer
    ...
    def get_queryset(self):
        ...
        attendee_uuid_list = set(
            Order.objects.filter(event=event).filter(balance_credit__gt=F('balance')).values_list(
        ...
        return EventAttendee.objects.filter(uuid__in=debitor_attendee_uuid_list)

    def list(self, request, *args, **kwargs):
        response_data = super().list(request, *args, **kwargs)

        return Response({'code': getattr(settings, 'SUCCESS_CODE', 1),
                         'message': 'Event Debitor List   is fetched successfully',
